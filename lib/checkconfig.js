        ...
        function (cb) {
            ...
            fs.stat(config.steviaDir, function (err, stats) {
                if (err != null) {
                    ...
                    var installedTestToolPath = path.join(config.steviaDir, config.toolsPath, 'tools-example', 'test-tool');
                    if (!shell.test('-d', installedTestToolPath)) {
                        var testToolPath = path.join(__dirname, '..', 'tools-example', 'test-tool');
                        shell.cp("-r", testToolPath, path.join(config.steviaDir, config.toolsPath))
                    }
                    cb(null);
                }
            });
